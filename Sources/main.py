...
env = gym.make(params.env_name) # we create an optimized environment thanks to universe
shared_model = AC_netwok(400*600, np.int16(env.action_space)) # shared_model is the model shared by the different agents (different threads in different cores)
...
optimizer = torch.optim.Adam(shared_model, params.lr) # the optimizer is also shared because it acts on the shared model
