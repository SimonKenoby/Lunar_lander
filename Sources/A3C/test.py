...
def test(rank, params, shared_model):
    ...
    env = gym.Env(params.env_name, video=True) # running an environment with a video
    env.seed(params.seed + rank) # asynchronizing the environment
    model = AC_netwok(env.observation_space.shape[0], env.action_space) # creating one model
